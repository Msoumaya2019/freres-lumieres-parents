...
| `deviceTokens/{token}`                | lecture réservée aux Cloud Functions ; écriture de soi si `uid` **et** `orgId` correspondent ; `audienceKeys` vide à la création puis figé — le serveur seul l'écrit                                                                                                                     |
